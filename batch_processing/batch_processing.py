...
def CreateProcessWindow(Window, ExistingIds):
    ...
    def ConfirmProcess():
        ...
        try:
            Num1 = int(Num1Entry.get().strip())
            Num2 = int(Num2Entry.get().strip())
            Time = int(TimeEntry.get().strip())
        except ValueError:
...
def ExecuteBatchWindow(Batch, BatchNumber, PendingBatches, CompletedProcesses, Window, TotalTime):
    ...
    while Batch:
        ...
        for ElapsedTime in range(Process["Time"]):
...
def RunProgram():
    ...
    def ExecuteBatches():
        nonlocal TotalTime
        if not Batches:
            messagebox.showwarning("Advertencia", "No hay lotes para ejecutar.")
            return

        BatchNumber = 1
        while Batches:
            CurrentBatch = Batches.pop(0)
            PendingBatches = len(Batches)
            TotalTime = ExecuteBatchWindow(CurrentBatch, BatchNumber, PendingBatches, CompletedProcesses, ExecutionWindow, TotalTime)
            BatchNumber += 1

    #Botón de inicio
    tk.Button(ExecutionWindow, text="Iniciar ejecución", command=ExecuteBatches).pack(pady=20)
    ExecutionWindow.mainloop()
